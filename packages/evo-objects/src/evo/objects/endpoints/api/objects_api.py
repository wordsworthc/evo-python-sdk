...
class ObjectsApi:
    ...
    async def list_object_version_ids(
        ...
        Request to fetch the latest version IDs for a collection of geoscience objects (maximum of 500). The request body must only contain a list of UUIDs of the objects to check.
    ...
    async def list_objects(
        ...
        :param deleted: (optional) When true, only objects that have been deleted will be returned
            Example: `True`
        :param order_by: (optional) A comma separated list of fields to order by. The default sort order is ascending.To specify the sort order, prefix the field name with either 'asc:' or 'desc:' for ascending or descending respectively.
            Example: `['name','desc:created_by']`
