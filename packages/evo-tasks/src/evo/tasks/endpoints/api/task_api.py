...
class TaskApi:
    """API client for the task endpoint.
    ...
    async def execute_task(
        ...
        """Triggers an asynchronous execution of a task

        Trigger an asynchronous task within a specific topic for a designated organization. The task, topic, and organization ID are supplied as path parameters. A JSON request body, referencing the **dict** schema and containing task-specific parameters, is required for this operation. Upon successful start, a 303 response is returned. The URL of the status endpoint for the newly created job is indicated in the Location header of the response.
    ...
    async def execute_task_sync(
        ...
        Start a task within a specific topic for a designated organization. The task, topic, and organization ID are supplied as path parameters. A JSON request body, referencing the **dict** schema and containing task-specific parameters, is required for this operation. Upon successful start, a 200 response is returned. The response body contains the results of the task.
        ...
        return await self.connector.call_api(
            method=RequestMethod.POST,
            resource_path="/compute/orgs/{org_id}/{topic}/{task}-sync",
            path_params=_path_params,
            header_params=_header_params,
            body=task_parameters,
            collection_formats=_collection_formats,
